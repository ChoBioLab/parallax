 ...
 */
include { ChannelUtils } from './lib/ChannelUtils'
...
params.validate_inputs = true
params.force_cpu = false
params.max_gpus_per_process = 1
...
def helpMessage() {
    ...
    Other options:
      ...
      --multiqc_config [file]         Custom config file to supply to MultiQC
      --multiqc_title [str]           Custom title for the MultiQC report
...
include { CHECK_GPU_AVAILABILITY } from './modules/local/check_gpu_availability'
...
if (params.validate_dependencies) {
    // Run dependency check before starting
    def check_result = "python ${projectDir}/bin/check_dependencies.py".execute()
    check_result.waitFor()
    if (check_result.exitValue() != 0) {
        error "Dependency check failed: ${check_result.err.text}"
    }
}
